...
/**
 * this one is better than give a listener to runner 
 * easier
 */

public class  MMInstrumentationRunner2 extends
	...
	void recordTestResult(int resultCode, Bundle results) throws IOException {
		...
		if (resultCode != 0) {
			...
			if (stack != null) {
				...
				message = stack.substring(index+1);
				this.mTestSuiteSerializer.attribute(null, "message", reason);
				// this.mTestSuiteSerializer.attribute(null, "type", reason);
				// this.mTestSuiteSerializer.text(stack);
				this.mTestSuiteSerializer.text(type);
				this.mTestSuiteSerializer.endTag(null, "result");
				if(message.length() != 0){
					this.mTestSuiteSerializer.startTag(null, "stack");
					this.mTestSuiteSerializer.text(message);
					this.mTestSuiteSerializer.endTag(null, "stack");
				}
			}
		} else {
			this.mTestSuiteSerializer
